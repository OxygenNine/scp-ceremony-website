...
6. **2026 单品的收录标准**：标题或简介里自行标注为「新春会单品 / 节目」的才收。未自我标注的同期投稿不收，宁可少收。
...
10. 浏览器自动化核验走 msedge（Playwright `channel="msedge"`），回归脚本在 `.workbuddy/scripts/e2e_check.py`，改完页面要跑一遍。截图脚本 `.workbuddy/scripts/pageshot.py`。
